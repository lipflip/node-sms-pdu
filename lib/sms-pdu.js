...
class SmsPdu {
  ...
  * ---------------------------------------------------------------- */
  generateSubmit(number, text, options) {
    ...
    let international = true; //force international format
    if (/^\+/.test(number)) {
    ...
    let vp = 168;
    let _interval = 24; //1 day default
    if ('validity' in options) {
      if (options.validity) {
        _interval = options.validity;
      }
    }

    if(_interval){
      var minutes = Math.ceil(_interval / 60),
          hours   = Math.ceil(_interval / 60 / 60),
          days    = Math.ceil(_interval / 60 / 60 / 24),
          weeks   = Math.ceil(_interval / 60 / 60 / 24 / 7);

      if(hours <= 12){
        vp = Math.ceil(minutes/5)-1;
      } else if(hours <= 24){
        vp =  Math.ceil((minutes-720)/30)+143;
      } else if(hours <= (30*24*3600)) {
        vp =  days+166;
      } else {
        vp = (weeks > 63 ? 63 : weeks)+192;
      }
    }

    let vpf = vp.toString().toString(16);
    byte_list.push(vpf);


    /*let srr = 0x01;
    let pdu_srr = (udhi << 5) | srr;
    byte_list.push(pdu_srr);*/
